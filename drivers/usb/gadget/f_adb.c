...
struct timer_list adb_flow_query_timer;

/* timer to delete the upper timer if it is still active */
struct timer_list adb_reactivate_expire_timer;


void adb_reactivate(void);

struct delayed_work adb_disable_work;
struct delayed_work adb_enable_work;
...
static void adb_reactivate_expire_function(unsigned long v)
{
  ...
  if(adb_flow.query_num <= ADB_INACTIVE_THRESHHOLD) {
    USB_PR("ADB reactivate: adb_flow_query_timer is active. Delete it.\n");
    del_timer(&adb_flow_query_timer);
  }
  else {
    USB_PR("ADB reactivate: adb_flow_query_timer is already inactive.\n");
  }
}
...
#ifdef CONFIG_USB_AUTO_INSTALL
			if(adb_flow.query_num <= ADB_INACTIVE_THRESHHOLD) {
			  adb_flow.read_num += xfer;  // data number we get at a time
			}
			adb_flow.active = 1;
#endif  /* CONFIG_USB_AUTO_INSTALL */
...
void adb_function_enable(int enable)
{
	...
	if (dev) {
		DBG(dev->cdev, "adb_function_enable(%s)\n",
			enable ? "true" : "false");
		#ifdef CONFIG_USB_AUTO_INSTALL
		USB_PR("ADB reactivate: %s, adb_function_enable(%s)\n", __func__, enable ? "true" : "false");
		#endif
