...
{	
       int rc = 0;
       CDBG("mt9d113_back_reset_init------------begin\n");
	...
	*/
	...
	if (!rc) {
       ...
       rc = gpio_direction_output(dev->sensor_reset, 1);
	mdelay(5);
       CDBG("mt9d113_back_reset_init------------end\n");
	return rc;
}
#endif
...
static int mt9d113_back_sensor_init_done(const struct msm_camera_sensor_info *data)
{
	   ...
	   CDBG("mt9d113_back_sensor_init_done-------------------------------begin\n");
	...
	*/
	...
	CDBG("mt9d113_back_sensor_init_done-------------------------------end\n");
	return 0;
}
...
static long mt9d113_back_set_sensor_mode(int mode)
{
	...
	CDBG("--------mt9d113_back_set_sensor_mode start mode=%d---------\n",mode);
	switch (mode) {
...
static int mt9d113_back_sensor_init_probe_init_power_on(const struct msm_camera_sensor_info *data)
{
	...
	CDBG("init entry ----------mt9d113_back_sensor_init_probe_init_power_on---------begin-----\n");
...
{
	int32_t rc = 0;
	CDBG("-----mt9d113_back_probe_init------\n");
	rc = i2c_add_driver(&mt9d113_back_i2c_driver);
	CDBG("-----mt9d113_back_probe_init------i2c_add_driver --- rc=%d\n",rc);
	if (rc < 0 || mt9d113_back_client == NULL) {
		...
		rc = -ENOTSUPP;
		CDBG("error----ENOTSUPP---------goto probe_done;");
		goto probe_done;
	}


	msm_camio_clk_rate_set(24000000);//
	CDBG("-----back msm_camio_clk_rate_set 24000000------\n");
