...
{			
	 ...
	 reg_write(this_gs_data,GS_ADI_REG_INT_ENABLE,0xe0);  
        reg_read(this_gs_data, GS_ADI_REG_INT_SOURCE);
...
{
		reg_write(this_gs_data,GS_ADI_REG_INT_ENABLE,0x00) ; 
		reg_write(this_gs_data,GS_ADI_REG_POWER_CTL,0x14);  
	   if (this_gs_data->use_irq)
...
{	
	int ret;
	ret = reg_write(this_gs_data,GS_ADI_REG_OFSX,0x03);   
	if (ret < 0) {
		printk(KERN_DEBUG "reg_write  GS_ADI_REG_OFSX failed\n");
	}
	ret = reg_write(this_gs_data,GS_ADI_REG_OFSY,0x03);	
	if (ret < 0) {
		printk(KERN_DEBUG "reg_write  GS_ADI_REG_OFSY failed\n");
	}

	ret = reg_write(this_gs_data,GS_ADI_REG_OFSZ,0x0B);	
