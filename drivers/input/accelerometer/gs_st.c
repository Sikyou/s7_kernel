...
#define Z_TOP_DIRECTION  1   // {1, -1}

#define SIN(x) (x == 90 ? 1 : (x == 270 ? -1 : 0))
#define COS(x) (x == 0 ? 1 : (x == 180 ? -1 : 0))
...
#define MG_PER_SAMPLE   720                                              
#define FILTER_SAMPLE_NUMBER  256             
...
{
	int status;	
	int x,y,z,x_ajusted,y_ajusted,z_adjusted;      
	...
	if (status & (1<<3))
	{
		...
		x_ajusted  = (x * COS(X_AZIMUTH) + y * SIN(X_AZIMUTH)); 
		#ifdef GS_SENSOR_HW_T3
		y_ajusted  = (y * COS(X_AZIMUTH) - x * SIN(X_AZIMUTH)) * Y_MIRROR;
		#else
		y_ajusted  = (y * COS(X_AZIMUTH) - x * SIN(X_AZIMUTH)) * Z_TOP_DIRECTION; 
		#endif
		z_adjusted = Z_TOP_DIRECTION * z; 
		...
		#endif
#if 0
	printk(KERN_ALERT "[gs_work_func] report ABS_X:%d, ABS_Y:%d, ABS_Z:%d\n", x, y, z);
#endif
...
{	
    ...
    printk(KERN_INFO "[%s,%d] client->name=%s, client->addr=0x%x\n", 
	...
	gs->input_dev->id.vendor = GS_ST35DE;
...
static int gs_suspend(struct i2c_client *client, pm_message_t mesg)
{
	...
	reg_write(gs, GS_ST_REG_CTRL3, 0); 
	
	reg_write(gs, GS_ST_REG_CTRL1, 0x00); 
