...
/* according to 80_VP796-1, should be: 144K, 400K, 15M, 20M, 25M, 49.152M */
...
static unsigned int msmsdcc_wifi_fmid = 24576000;
static unsigned int msmsdcc_wifi_fmax = 49152000;
static unsigned int msmsdcc_wifi_pwrsave = 1;//0;//
#endif
...
msmsdcc_set_ios(struct mmc_host *mmc, struct mmc_ios *ios)
{
	...
	if (ios->clock) {
        ...
        DBG(host, "ios->clock = %u host->clk_rate = %u\n", ios->clock, host->clk_rate);

#if HWVERID_HIGHER(S70, T1)
		if (host->plat->embedded_sdio) {
			if ((ios->clock < msmsdcc_wifi_fmax) && (ios->clock > msmsdcc_wifi_fmid))
				ios->clock = msmsdcc_wifi_fmid;
        } else {
		...
		if (ios->clock != host->clk_rate) {
            DBG(host, "changeto: ios->clock = %u\n", ios->clock);
			rc = clk_set_rate(host->clk, ios->clock);
			if (rc < 0) {
                DBG(host, "failed and change to: msmsdcc_temp = %u\n", msmsdcc_temp);
				rc = clk_set_rate(host->clk, msmsdcc_temp);
				WARN_ON(rc < 0);
                if (rc < 0) {
                    DBG(host, "failed to change: msmsdcc_temp = %u\n", msmsdcc_temp);
                }
...
msmsdcc_probe(struct platform_device *pdev)
{
	 ...
	 */
	if (plat->status_irq) {
		DBGSDCC("request irq %d", plat->status_irq);
	...
	printk(KERN_INFO "%s: MMC clock %u -> %u Hz, PCLK %u Hz\n",
	       mmc_hostname(mmc), msmsdcc_fmin, 
	       plat->embedded_sdio ? msmsdcc_wifi_fmax : msmsdcc_fmax, 
	       host->pclk_rate);
#else
    printk(KERN_INFO "%s: MMC clock %u -> %u Hz, PCLK %u Hz\n",
    ...
    printk(KERN_INFO "%s: Power save feature enable = %d\n",
           mmc_hostname(mmc), 
           plat->embedded_sdio ? msmsdcc_wifi_pwrsave : msmsdcc_pwrsave);
#else
	printk(KERN_INFO "%s: Power save feature enable = %d\n",
