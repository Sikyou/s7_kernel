...
static void AKECS_Reset(void)
{
  ...
  // NOTICE(suchangyu): have a test, set gpio 19 high
	      ...
	      unsigned int cmd, unsigned long arg)
{
	...
	case ECS_IOCTL_APP_SET_AFLAG:
	  // NOTICE: suchangyu adds debug
	  #if DEBUG
	  printk(KERN_ALERT "[akm_aot_ioctl] set a_flag from user, flag:%d\n", flag);
	  #endif
...
#endif
		break;
/*BK4D00263, delet gsensor interface, dingxifeng KF14049, 2009-5-21 begin */
...
#endif
  /* request gpio HERE */
  //TODO: move it to board-XXXXX
  #if 0
  int ret = 0;    
  ret = gpio_request(COMPASS_RST, "Compass RESET Enable");
  if (ret) {
    printk("***suchangyu**AKECS_Reset* :gpio_request failed.\n");
    return -1;
  }

  ret = gpio_tlmm_config(GPIO_CFG(COMPASS_RST, 0, GPIO_OUTPUT, GPIO_NO_PULL, GPIO_10MA), GPIO_ENABLE);
  if(ret < 0) {
    printk("***suchangyu**AKECS_Reset* : gpio_tlmm_config failed.\n");
    gpio_free(COMPASS_RST);
    //return -1;
  }
  // need to be delete. suchangyu
