...
#define INIT_IMAGE_FILE_720P "/initlogo_720p.rle"
#define INIT_IMAGE_FILE "/initlogo.rle"
//#define INIT_IMAGE_FILE "/data/initlogo.rle"

extern int load_565rle_image(char *filename);
#endif

#ifdef CONFIG_MSM_HDMI
extern int hdmi_get_plug_status(void);
#endif
...
static int msm_fb_probe(struct platform_device *pdev)
{
    ...
    /* Begin: modify by w00163571 2010-7-5 for set backlight down in powerup sequence */
//	mfd->bl_level = mfd->panel_info.bl_max;
	mfd->bl_level = (mfd->panel_info.bl_max - mfd->panel_info.bl_min) * 7 / 10 
					+ mfd->panel_info.bl_min;
    /* End: added by w00163571 2010-7-5 for set backlight down in powerup sequence */
...
static int msm_fb_register(struct msm_fb_data_type *mfd)
{
	...
	/* add width/height arrording to AT070TN92 specification. suchangyu. 20100628. begin */
	var->height = 86; //-1,	/* height of picture in mm */
	var->width = 154; //-1,	/* width of picture in mm */
	/* add width/height arrording to AT070TN92 specification. suchangyu. 20100628. end */
	...
	default:
    	...
    	fix->smem_len = panel_info->xres * panel_info->yres * bpp * mfd->fb_page;
    ...
    {
        if (!load_565rle_image(INIT_IMAGE_FILE_720P)) ; /* Flip buffer */
    }
    else
    {
#endif
...
static int msm_fb_open(struct fb_info *info, int user)
{
	...
	if (!mfd->ref_cnt) {
		...
		msm_fb_set_backlight(mfd,25, 0);
		mdelay(100);

		msm_fb_set_backlight(mfd,50, 0);
		mdelay(100);

		msm_fb_set_backlight(mfd,level, 0);
        mfd->bl_level = level;
	}
