...
#define SENSOR_POLLING_JIFFIES (80)   //modify the time from 10 to 80 (10)
#define MOD_SENSOR_POLLING_JIFFIES 1*HZ   // modify the time from 95 to 200 (10)
	...
	unsigned long param)
{
	...
	{	
		...
		case SENSORS_GET_LUX_FIR:
			printk("!!!!!!!!!!!!!!!!!!!SENSORS_GET_LUX_FIR,param=%d\n",param);
...
static void Po188_resume()
{
	if( po188_driver.status_on )
	{
		mod_timer(&po188_driver.timer, jiffies + 3*MOD_SENSOR_POLLING_JIFFIES);					
...
po188_init(void)
{
	...
	PO188_DMSG("PO188_init, SENSOR_POLLING_JIFFIES=%d",SENSOR_POLLING_JIFFIES);
